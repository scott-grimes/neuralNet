...
class NeuralNet(object):
  ...
  def SGD(self, trainingData, epochs, miniBatchSize, eta, testData = None):
    ...
    for i in xrange(epochs):
      ...
      if testData is not None:
        pcnt = self.evaluate(testData)*1.0/nTest
        print( "Epoch {0}: {1}%".format( i , round(pcnt*10000)/100) )
      else:
  ...
  def backprop(self,a,expectedOut):
    ...
    delta = self.costDerivative(inputs[-1], expectedOut) * sigmoidPrime(zs[-1])
    nablaB[-1] = delta
    nablaW[-1] = np.dot( delta, inputs[-2].transpose() )
    ...
    for i in xrange(2,self.numLayers):
      ...
      nablaB[-i] = delta
      nablaW[-i] = np.dot( delta, inputs[-i-1].transpose() )
  ...
  def updateMiniBatch(self, miniBatch, eta):
    ...
    self.biases = [b-diff*nb for b, nb in zip(self.biases, nablaB)]
  ...
  def evaluate(self, testData):
    ...
    return sum( actualOut==expectedOut  for (actualOut, expectedOut ) in testResults)
